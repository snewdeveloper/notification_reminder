...
import com.greenlime.notification_reminder_flutter.utils.KillerManagerUtils;
...
public class DialogKillerManagerBuilder {
    ...
    public void show() {
        ...
        if (!KillerManager.isActionAvailable(mContext, mAction)) {
            LogUtils.i(this.getClass().getName(), "This action is not available for this device. No need to show dialog.");
            return;
        }

        if (KillerManager.getDevice() == null) {
            LogUtils.i(this.getClass().getName(), "Device not in the list. No need to show dialog.");
            return;
        }

        // Already marked "Don't show again"
        if (enableDontShowAgain && KillerManagerUtils.isDontShowAgain(mContext, mAction)) {
            return;
        }

        String positiveText = (positiveBtnStr != null) ? positiveBtnStr :
                mContext.getText(R.string.dialog_button).toString();

        String negativeText = (negativeBtnStr != null) ? negativeBtnStr :
                mContext.getText(android.R.string.cancel).toString();

        // Inflate custom view
        View customView = LayoutInflater.from(mContext)
                .inflate(R.layout.md_dialog_custom_view, null);

        // Create AlertDialog
//        AlertDialog.Builder builder = new AlertDialog.Builder(mContext);
        Context themedContext = new ContextThemeWrapper(mContext, com.google.android.material.R.style.Theme_Material3_Light_Dialog_Alert);
...
//        MaterialAlertDialogBuilder builder = new MaterialAlertDialogBuilder(mContext);
        ...
        if (enableDontShowAgain) {
            dontShowCheck = new CheckBox(mContext);
            dontShowCheck.setText(R.string.dialog_do_not_show_again);
            ((ViewGroup) customView).addView(dontShowCheck);

            dontShowCheck.setOnCheckedChangeListener((button, isChecked) ->
...
//        alertDialog.show();
        ...
        dialog.show();
        // Initialize custom view if needed
        initView(customView);
    }
